...
const aviConverter = () => {
    ...
    const transcode = async () => {
        ...
        try {
            ...
            await ffmpeg.exec(['-i', 'input', 'output.avi']);
            
            const data = await ffmpeg.readFile('output.avi');
            const videoURL = URL.createObjectURL(new Blob([data.buffer], { type: 'video/avi' }));
            setDownloadLink(videoURL);

            // Automatically trigger download
            triggerDownload(videoURL, 'output.avi');
        } catch (error) {
            ...
            {loaded && (
                <div className="actions">
                    {processing ? (
                        ...
                        <button className="merge-btn" onClick={transcode}>Convert to avi</button>
                    )}
                </div>
            )}
        </div>
